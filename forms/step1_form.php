...
/**
 * File containing step 1 of the upload form
 *
...
/**
 * Uploads a CSV file of badge recipient information.
 *
 * @package     block_badgeawarder
 * @copyright  2013 Learning Technology Services, www.lts.ie - Lead Developer: Bas Brands
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class block_badgeawarder_step1_form extends moodleform {
    ...
    public function definition() {
        ...
        if (!empty($config->showextendedoption) && ($config->showextendedoption == 1)) {
            $this->add_extended_options($mform, $config);
        } else {
            $this->add_default_hidden_fields($mform, $config);
        }
    ...
    /**
     * Adds the delimiter, encoding, preview-rows, and mode elements shown when the admin has
     * enabled extended options, letting the uploader choose them instead of the site defaults.
     *
     * @param \MoodleQuickForm $mform the form to add elements to.
     * @param \stdClass $config the plugin's admin settings.
     * @return void
     */
    private function add_extended_options($mform, $config) {
    ...
    /**
     * Adds hidden delimiter, encoding, preview-rows, and mode elements carrying the admin's
     * configured defaults, used when extended options are turned off so the uploader can't
     * override them.
     *
     * @param \MoodleQuickForm $mform the form to add elements to.
     * @param \stdClass $config the plugin's admin settings.
     * @return void
     */
    private function add_default_hidden_fields($mform, $config) {
        // Set delimiter.
        if (!empty($config->defaultdelimiter)) {
